...
The **Air-Lock** is a validation gate inside the orchestrator that checks every agent step's output against its declared `output.schema` before passing it downstream. See [docs/architecture/air-lock.md](architecture/air-lock.md) for the full explanation.
...
Secrets must be marked `secret: true` at every layer they pass through — from the workflow `env` block through agent params to server params — so the runtime can redact them from logs and the run envelope. See [docs/concepts/variables.md](concepts/variables.md#secrets) for the full propagation rules and an end-to-end example.
