...
**Four container tiers, clearly separated.** Orchestrator, Agent Runtime, LLM Gateway, and the shipped envelope tool server are distinct tiers with distinct responsibilities. The envelope server is a first-party Docker image managed by Kimitsu — it has a back-channel to the orchestrator and is the orchestrator's read interface for agents. User-provided tool servers have no orchestrator dependency and are operator-managed. The architecture diagram makes this split explicit.
